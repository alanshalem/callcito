  ...
  return {
    ...
    model: {
      ...
      tools: [
        ...
        toolDef("add_to_cart", "Agrega un producto al carrito. OBLIGATORIO pasar productId (UUID) del producto que el cliente eligió. Obtené el productId del último search_products result (campo 'id'). NUNCA llames sin productId.", {
          type: "object",
          properties: {
            productId: { type: "string", description: "UUID del producto tomado del último search_products result. Obligatorio." },
            quantity: { type: "number", default: 1, description: "Cantidad. Default 1." },
          },
          required: ["productId"],
        }, c.baseUrl),
